...
const PaginationButton = styled.button<PaginationButton>`
...
function Pagination({ dataLength }: PaginationProps) {
  ...
  const totalPage = Math.ceil(dataLength / PAGE_SIZE)
  ...
  return (
    <StyledPagination>
      <P>
        ...
        <span>{currentPage === totalPage ? dataLength : currentPage * PAGE_SIZE}</span> of <span>{dataLength}</span>{' '}
        results
      </P>
      ...
      <Buttons>
        ...
        <PaginationButton onClick={nextPage} disabled={currentPage === totalPage}>
          <span>Next</span>
          <HiChevronRight />
        </PaginationButton>
      </Buttons>
